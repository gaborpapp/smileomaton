...
bool SoundBuffer::LoadFromFile(const std::string& Filename)
{
    ...
    std::auto_ptr<SoundFile> File(SoundFile::CreateRead(Filename));
...
{
    // Create the sound file
    std::auto_ptr<SoundFile> File(SoundFile::CreateRead(Data, SizeInBytes));
...
bool SoundBuffer::SaveToFile(const std::string& Filename) const
{
    ...
    std::auto_ptr<SoundFile> File(SoundFile::CreateWrite(Filename, GetChannelsCount(), GetSampleRate()));
    if (File.get())
    {
...
bool SoundBuffer::Update(unsigned int ChannelsCount, unsigned int SampleRate)
{
    ...
    ALenum Format = AudioDevice::GetInstance().GetFormatFromChannelsCount(ChannelsCount);
